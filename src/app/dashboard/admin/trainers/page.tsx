...
import { Dumbbell, Plus, Pencil, Trash2, X, CheckCircle2, AlertCircle } from 'lucide-react';
...
interface User {
  ...
  role: UserRole;
  createdAt: string;
  trainer: { id: string; active: boolean; bio: string | null; specialties: string[] } | null;
}
...
export default function AdminTrainersPage() {
  ...
  async function fetchUsers() {
    ...
    setUsers(data.filter((u: User) => u.role === UserRole.TRAINER));
    setLoading(false);
  }

  function resetForm() {
    setForm({ name: '', email: '', phone: '', password: '', bio: '', specialties: '', active: true });
    setEditing(null);
    setShowForm(false);
  }
  ...
  function startEdit(user: User) {
    ...
    setForm({
      ...
      specialties: user.trainer?.specialties?.join(', ') || '',
      active: user.trainer?.active ?? true,
    });
    setShowForm(true);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    delete payload.specialtiesInput;
    ...
    setSuccess(editing ? 'Trainer updated' : 'Trainer created');
    resetForm();
    fetchUsers();
  }
  ...
  async function deactivate(id: string) {
    ...
    if (res.ok) {
      setSuccess('Trainer deactivated');
      fetchUsers();
    } else {
  ...
  if (loading) return <Spinner label="Loading trainers..." />;

  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        title="Manage Trainers"
        subtitle="View, add, edit, and deactivate personal trainer accounts."
        action={
      ...
      {showForm && (
        <div className="card">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="section-title">{editing ? 'Edit Trainer' : 'Add Trainer'}</h2>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              ...
              <input
                ...
                className="input"
                placeholder="Strength, HIIT, Weight Loss"
              />
            </div>
            ...
            <div className="sm:col-span-2">
              <button type="submit" className="btn-primary">
                {editing ? 'Update Trainer' : 'Create Trainer'}
              </button>
            </div>
          </form>
        </div>
              ...
              {users.map((user) => (
                <tr key={user.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/60">
                  ...
                  <td className="table-cell dark:text-slate-300">
                    {user.trainer?.specialties?.join(', ') || '—'}
                  </td>
                  <td className="table-cell">
                    <span
                      className={`badge ${user.trainer?.active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400'}`}
                    >
                      {user.trainer?.active ? 'Active' : 'Inactive'}
                    </span>
                  </td>
